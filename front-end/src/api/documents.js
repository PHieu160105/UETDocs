...
export const documentAPI = {
  ...
  getMyUploads: (params = {}) => authAPI.get('/documents/me/uploads', { params }),
  getMyVotes: (params = {}) => authAPI.get('/document-votes/me', { params }),
  getDocumentInteraction: (id) => authAPI.get(`/documents/${id}/interaction`),
  voteDocument: (id, vote) => authAPI.put(`/documents/${id}/vote`, { vote }),
  clearDocumentVote: (id) => authAPI.delete(`/documents/${id}/vote`),
  reportDocument: (id, reason) => authAPI.post(`/documents/${id}/report`, { reason }),
  getMyDownloads: (params = {}) => authAPI.get('/downloads', { params }),
  getMyBookmarks: (params = {}) => authAPI.get('/bookmarks', { params }),
  createBookmark: (documentId) => authAPI.post(`/bookmarks/${documentId}`),
  deleteBookmark: (documentId) => authAPI.delete(`/bookmarks/${documentId}`),
  getMyCourses: (params = {}) => authAPI.get('/courses', { params }),
  getCourseDetail: (courseId) => authAPI.get(`/courses/${courseId}`),
  createCourse: (payload) => authAPI.post('/courses', payload),
  addDocumentToCourse: (courseId, documentId) => authAPI.post(`/courses/${courseId}/documents/${documentId}`),
  removeDocumentFromCourse: (courseId, documentId) => authAPI.delete(`/courses/${courseId}/documents/${documentId}`),
  getDocumentCourseMemberships: (id) => authAPI.get(`/documents/${id}/courses/me`),
